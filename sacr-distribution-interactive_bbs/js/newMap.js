...
function getData() {
    ...
    // Add Circle Data
    $.ajax("data/bbsDataByYear.geojson", {
        beforeSend: function(xhr) {
            if (xhr.overrideMimeType) {
                xhr.overrideMimeType("application/json");
            }
        ...
        error: function() {
            //Let the user know if they cannot load the data.
            alert('There has been a problem loading the data. filename: bbsDataByYear.geojson');
        }
    });
...
function updatePropSymbols(year) {
    ...
    mylayer.eachLayer(function(layer) {
        ...
        var myPopup = popContent(value, props.site_id, year)
        if (myPopup != "unbind") {
...
function popContent(value, site_id, year) {
    // format value of attributes depending on the
    var popupContent = "";
    var valuestring;
    if (value == null) {
        ...
        popupContent += '<div id="popup_title">' + site_id + '</div>'
        popupContent += '<div id=popup_content">' + valuestring + ' in ' + year + '</div>';
        return popupContent;
    } else {
        valuestring = value.toLocaleString('en') + ' Sandhill Crane pairs Counted';
        popupContent += '<div id="popup_title">' + site_id + '</div>'
        popupContent += '<div id=popup_content">' + valuestring + ' in ' + year + '</div>';
        return popupContent;
    }
...
document.getElementById("buttonOutText").innerHTML = "Show all BBS circles";
...
function togglePoints() { //funcion that performs an on or off of data based on its state
    if (!toggle) {
        ...
        document.getElementById("buttonOutText").innerHTML = "Hide No-Count BBS Locations ";
        updatePropSymbols(userYear);
    } else {
